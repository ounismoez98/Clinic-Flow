...
export class FacturesComponent implements OnInit {
  ...
  get stats() {
    ...
    return [
      { label: 'Total Invoices', value: String(total), color: 'text-blue-600 bg-blue-50', icon: 'M9 14l6-6m-5.5.5h.01m4.99 5h.01M19 21V5a2 2 0 0 0-2-2H7a2 2 0 0 0-2 2v16l3.5-2 3.5 2 3.5-2 3.5 2z' },
      { label: 'Paid', value: String(payees), color: 'text-green-600 bg-green-50', icon: 'M9 12.75 11.25 15 15 9.75M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0z' },
      { label: 'Unpaid', value: String(nonPayees), color: 'text-orange-600 bg-orange-50', icon: 'M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126zM12 15.75h.007v.008H12v-.008z' },
      { label: 'Revenue', value: totalTTC.toFixed(2) + ' DT', color: 'text-teal-600 bg-teal-50', icon: 'M2.25 18.75a60.07 60.07 0 0 1 15.797 2.101c.727.198 1.453-.342 1.453-1.096V18.75M3.75 4.5v.75A.75.75 0 0 1 3 6h-.75m0 0v-.375c0-.621.504-1.125 1.125-1.125H20.25M2.25 6v9m18-10.5v.75c0 .414.336.75.75.75h.75m-1.5-1.5h.375c.621 0 1.125.504 1.125 1.125v9.75c0 .621-.504 1.125-1.125 1.125h-.375m1.5-1.5H21a.75.75 0 0 0-.75.75v.75m0 0H3.75m0 0h-.375a1.125 1.125 0 0 1-1.125-1.125V15m1.5 1.5v-.75A.75.75 0 0 0 3 15h-.75M15 10.5a3 3 0 1 1-6 0 3 3 0 0 1 6 0zm3 0h.008v.008H18V10.5zm-12 0h.008v.008H6V10.5z' },
    ];
  }
    ...
    if (this.modalMode === 'create') {
      this.http.post<Facture>(API, this.form).subscribe({
        next: f => {
          ...
          this.closeModal();
          this.flash('Invoice created successfully!');
        },
      ...
      if (this.form.statut === 'PAYEE') {
        this.http.put<Facture>(`${API}/${this.form.id}/payer`, {}).subscribe({
          next: updated => {
            ...
            this.closeModal();
            this.flash('Invoice marked as paid!');
          },
          error: () => this.error = 'Error during update.'
        });
      } else {
        this.closeModal();
        this.flash('Status updated locally.');
      }
    }
  }
  ...
  doDelete() {
    ...
    this.http.delete(`${API}/${this.deleteTargetId}`).subscribe({
      ...
      error: () => { this.error = 'Error during deletion.'; this.showDeleteConfirm = false; }
    });
  }
